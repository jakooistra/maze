...
static void indicateStartEnd(Image *image, Maze const *maze, XY pos, bool isStart, int wallWidth, int cellWidth) {
...
std::unique_ptr<Image> convertToImage(Maze const *maze, int wallWidth, int cellWidth) {
    ...
    for (int cx = 0; cx < maze->getWidth(); ++ cx) {
        for (int cy = 0; cy < maze->getHeight(); ++ cy) {
            ...
            if (xy == maze->getStart()) {
                indicateStartEnd(image.get(), maze, xy, true, wallWidth, cellWidth);
            }
            if (xy == maze->getFinish()) {
                indicateStartEnd(image.get(), maze, xy, false, wallWidth, cellWidth);
            }
